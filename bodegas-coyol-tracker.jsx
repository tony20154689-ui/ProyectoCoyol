...
export default function App() {
  ...
  const [data, setData] = useState(() => { try { const d = window._bodegas_storage; return d && d.maestros && d.clientes ? d : initialData(); } catch { return initialData(); } });
  const [section, setSection] = useState("resumen");
  const [toast, setToast] = useState("");
  useEffect(() => { window._bodegas_storage = data; }, [data]);
  const resetData = () => { if (confirm("¿Restablecer datos originales?")) setData(initialData()); };
